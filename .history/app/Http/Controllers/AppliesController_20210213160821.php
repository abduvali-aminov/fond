...
class AppliesController extends Controller
{
    ...
    {
        $applies = Apply::latest()->where('status', '<',5)->where('status','>', 0)->get();
        $cause = $this->service->causes($request);
        $summ_type = $this->service->summ_type($request);

        return view('front.apply.index', compact('applies', 'cause', 'summ_type'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('front.apply.create', compact('causes', 'summ_type'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $cause = $this->service->summ_type($request);
        $summ_type = $this->service->causes($request);

        return view('front.cause.show', compact('apply', 'cause', 'summ_type'));
    }
