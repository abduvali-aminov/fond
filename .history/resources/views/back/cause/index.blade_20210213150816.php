            ...
            @foreach ($causes as $counter => $cause)
                <tr>
                    ...
                    <td class="text-right">
                        ...
                        <a href="{{ route('change-delete', $cause->id) }}" class="btn btn-danger" onclick="return confirm('Are you sure?')" class="btn btn-secondary">
                            <form action="{{ route('change-delete', $cause->id)}}" method="POST">
                            @csrf
                            @method('put')
